...
institution           = "MPIfR"
base_output_directory = "/hercules/results/aswin/results/PTTAPES1_res/archive_11/time_250/DDplan14"
...
singlepulse_plot_SNR  = 5.0  # threshold SNR for singlepulse plot
...
#    ddplans.append(dedisp_plan( 567.0,   3.0,      24,     7,       32,       8))
    ...
    # The values here are:       lodm dmstep dms/call #calls #subbands downsamp                                                            
# PMPS plan below
#ddplans.append(dedisp_plan(   0.0,   0.5,      12,    16,       16,       1))
#ddplans.append(dedisp_plan(  96.0,   1.0,      12,     6,       16,       2))
#ddplans.append(dedisp_plan( 168.0,   2.0,      12,     7,       16,       4))
#ddplans.append(dedisp_plan( 336.0,   5.0,      10,     8,       16,       8))
#ddplans.append(dedisp_plan( 736.0,  10.0,      10,     7,       16,      16))
#ddplans.append(dedisp_plan(1436.0,  20.0,      10,     3,       16,      32))

#New PMPS plan : Ralph : DDplan.py -l 0.0 -d 3000.0 -f 2934.0 -b 576 -n 192 -t 0.00025 -s 96
ddplans.append(dedisp_plan(   0.0,   2.0,      66,    11,       96,       1))
ddplans.append(dedisp_plan(1452.0,   3.0,      76,     3,       96,       2))
ddplans.append(dedisp_plan(2136.0,   5.0,      76,     3,       96,       4))




# test line RE
#ddplans.append(dedisp_plan(   0.0,   0.5,      10,     1,       96,       1))

#ddplans.append(dedisp_plan( 168.0,   2.0,      12,     7,       16,       4))

#   Low DM    High DM     dDM  DownSamp  dsubDM   #DMs  DMs/call  calls  WorkFract
#    0.000    302.400    0.30       1    3.60    1008      12      84    0.8235
#  302.400    488.400    0.50       2    6.00     372      12      31    0.152
#  488.400    608.400    1.00       4   12.00     120      12      10    0.02451

#ddplans.append(dedisp_plan(   480,   10.0,      24,    2,       16,       1))#modified 
#ddplans.append(dedisp_plan( 302.8,   0.5,      12,    31,       16,       2))
#ddplans.append(dedisp_plan( 488.4,   1.0,      12,    10,       16,       4))
    ...
    for ddplan in ddplans:
        ...
        for passnum in range(ddplan.numpasses):
            ...
            if use_subbands:
                ...
                cmd = "prepsubband -sub -subdm %s -downsamp %d -nsub %d -mask %s -zerodm -o subbands/%s %s > %s.subout"%\
                ...
                cmd = "prepsubband -ncpus 12 -mask %s -lodm %.2f -dmstep %.2f -numdms %d -numout %d -o %s %s"%\
            ...
            for dmstr in ddplan.dmlist[passnum]:
                ...
                cmd = "accelsearch -locpow -noharmpolish -numharm %d -sigma %f -zmax %d -flo %f %s"%\
